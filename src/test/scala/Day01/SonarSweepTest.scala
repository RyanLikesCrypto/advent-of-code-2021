...
class SonarSweepSpec extends AnyFlatSpec with should.Matchers {
  ...
  it should "pass the official example" in {
    val inputs = Source.fromResource("Day01/PartOne_OfficialExample.txt").getLines.toList.map(_.toInt)
    val result = SonarSweep.countIncreases(inputs)
    result should be(7)
  }

  it should "pass the official list" in {
    val inputs = Source.fromResource("Day01/PartOne_OfficialInput.txt").getLines.toList.map(_.toInt)
    val result = SonarSweep.countIncreases(inputs)
  ...
  it should "pass the official example" in {
    val inputs = Source.fromResource("Day01/PartTwo_OfficialExample.txt").getLines.toList.map(_.toInt)
    val result = SonarSweep.countSumIncreases(inputs)
    result should be(5)
  }

  it should "pass the official list" in {
    val inputs = Source.fromResource("Day01/PartTwo_OfficialInput.txt").getLines.toList.map(_.toInt)
    val result = SonarSweep.countSumIncreases(inputs)
